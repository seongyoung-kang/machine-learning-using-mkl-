...
int main(int argc, char **argv)
{
    ...
    mode = (int *)malloc(sizeif(int)*3); //mode는 openMP와 mkl 중에 어떤 수식이 더빠른지 저장해 놓습니다.
    ...
    init(net, conf_str,(void *) threads,(void*) mode); //네트워크구조체와 , json 형태의 배열을 보내줍니다. 네트워크 구조체 멤버들의 메모리크기 할당& 초기화

    reader(net); //train , test 에 input값을 넣어주는 역할을 합니다.

    train(net, (void *) threads,(void *) mode); //net에 있는 neuron 과 error에 적절한 값을 다 넣어준후 backpropagation train 을 합니다
